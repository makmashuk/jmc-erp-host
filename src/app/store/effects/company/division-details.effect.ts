...
// @Services
import { DivisionDetailsService } from 'app/modules/admin/hr-and-payroll/company/division-details-create/services/division-details.service';

@Injectable({
    providedIn: 'root',
})
export class DivisionDetailsEffect {
    ...
    constructor(
        ...
        private _service: DivisionDetailsService
    ) {}

    createNewDivisionDetails$ = createEffect(() => {
        return this._action$.pipe(
            ofType<fromAction.CreateNewDivisionDetails>(
                fromAction.DIVISION_DETAILS_ACTION.CREATE_NEW_DIVISION
            ),
            switchMap(({ formData: data }) => {
                const payload = this.normalizePayload(data);
                return this._service.createDivisionDetails(payload).pipe(
                    map((newDivision) => {
                        this.openSnackbar(
    ...
    getEntities$ = createEffect(() => {
        return this._action$.pipe(
            ofType<fromAction.GetListOfDivisions>(
                fromAction.DIVISION_DETAILS_ACTION.GET_LIST_OF_DIVISIONS
            ),
            switchMap(
                ({
                    pageNumber,
                    ...
                    return this._service
                        ...
                        .pipe(
                            map(({ data, count }) => {
                                ...
                                return new fromAction.GetListOfDivisionsSuccess(
                                    normalizeData,
                                    count
                                );
                            })
