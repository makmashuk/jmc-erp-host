...
} from 'app/store/actions/company/subsection-details.actions';
...
import {
    ...
    isSubSectionLoading,
    isSubSectionUpdated,
} from 'app/store/selectors/company/subsection-details.selector';
...
import { GetListOfSections } from 'app/store/actions/company/section-details.actions';
import { getSectionList } from 'app/store/selectors';

@Component({
    selector: 'app-subsection-details',
    templateUrl: './subsection-details.component.html',
    styleUrls: ['./subsection-details.component.scss'],
})
export class SubSectionDetailsComponent
    implements OnInit, AfterViewInit, OnDestroy
{
    ...
    constructor(
        ...
        this.totalDataCount$ = this._store.select(getSubSectionCount);
        this.dataLoading$ = this._store.select(isSubSectionLoading);
    }
    ...
    ngOnInit(): void {
        ...
        this.dataSource$ = this._store.select(getSubSectionList);
        ...
        this.sectionList$ = this._store.select(getSectionList);

        this._store.dispatch(new GetListOfSubSections(1, 10));
    ...
    onChangeDepartmentOption(key: string) {
        this._store.dispatch(
            new GetListOfSections(null, null, {
                company: this.formInstance.get('company_id').value,
                location: this.formInstance.get('location_id').value,
                unit: this.formInstance.get('unit_id').value,
                division: this.formInstance.get('division_id').value,
                department: this.formInstance.get('department_id').value,
            })
        );
    }
    ...
    handleCreateModal(type?: string) {
        if (type === 'edit') {
            ...
            this.onChangeDepartmentOption('department_id');
        }
    ...
    onCreate() {
        ...
        this._store.dispatch(new CreateNewSubSectionDetails(formInfo));
    ...
    onEdit() {
        ...
        this._store.dispatch(new UpdateSubSectionDetails(updated_data));
    ...
    onDelete() {
        ...
        this.isLoading = true;
        this._store.dispatch(new DeleteSubSectionDetails(id));
        this.handleCloseModal();
    }
